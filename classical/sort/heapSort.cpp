...
void heapSort(int data[], int length) {
    ...
    for (int i = length - 1; i > 0; --i) { // 每次选出（删去）堆中最大的元素（即堆顶），调整剩下元素的为大根堆
        swap(data[0], data[i]); // 将堆顶元素和最后一个元素交换
        adjustDown(data, 0, i - 1); // 排除最后一个元素，调整剩下的堆
    }
}
